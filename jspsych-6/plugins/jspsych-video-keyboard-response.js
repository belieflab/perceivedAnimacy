...
jsPsych.plugins["video-keyboard-response"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    let video_preload_blob = jsPsych.pluginAPI.getVideoBuffer(trial.sources[0]);
    if(!video_preload_blob) {
      for(let i=0; i<trial.sources.length; i++){
        let file_name = trial.sources[i];
        if(file_name.indexOf('?') > -1){
          file_name = file_name.substring(0, file_name.indexOf('?'));
        }
        let type = file_name.substr(file_name.lastIndexOf('.') + 1);
    ...
    if(trial.stop !== null){
      display_element.querySelector('#jspsych-video-keyboard-response-stimulus').addEventListener('timeupdate', function(e){
        let currenttime = display_element.querySelector('#jspsych-video-keyboard-response-stimulus').currentTime;
